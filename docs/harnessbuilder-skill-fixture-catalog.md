...
+ Space-level `.harness-builder/agents` overlay
```
...
额外的 Harness Space overlay 用来验证 `.harness-builder/agents/<agent>`，因为它不是 Skill，不能伪装成 Skill fixture。
...
| Human-owned source preservation | `.harness-builder/agents/codex` | `.harness-builder/agents/cursor` | `.harness-builder/agents/codebuddy` | `.harness-builder/agents/claude-code` |
...
- `.harness-agents/codex/.codex/commands` 不在正向 Skill 中；
...
- `.harness-builder/agents/claude-code` source 不变，根 `CLAUDE.md` 和 target settings 可完整重生成。
...
- 旧 `tagents/`、Space root `.harness-agents/` 与新 `.harness-builder/agents/` 单独或同时存在。
...
Harness Space `.harness-builder/agents/<agent>` 单独维护，并保持各平台原生配置树：
...
- Harness Space-only workspace-specific command/hook/rule；
