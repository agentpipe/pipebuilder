...
首版 E2 只使用 Codex。Runner 默认使用已安装 Codex 的有效默认模型，release job 可通过显式 `--model` 固定模型；不在仓库中硬编码可能下线的 model id。它只影响 E2，不进入 E0/E1，也不写入 Harness Space 产品协议。Cursor、CodeBuddy 和 Claude Code 首版完成 E0 投影；只有确认稳定官方入口后才加入 E1/E2。
