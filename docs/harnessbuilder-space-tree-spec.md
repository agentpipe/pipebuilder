# HarnessBuilder 通用 HSpace children Tree 协议
...
本文定义任意普通 HSpace 在自身目录内物理包含 N 个显式 child HSpaces 时的发现、构建、验证、
清理、并发和故障恢复契约。它是 `harness-space.v1` 之上的通用编排层，不改变成员的单 Space
协议，也不定义 Leader、Worker 或其他产品 role。

## 1. 拓扑与所有权

```text
root-space/                          # 普通 HSpace + HSpace Tree root
├── harness-space.json
├── root-space.code-workspace
├── harness-space-tree.json
├── .harness-builder/
│   ├── lock.json                    # root HSpace 的单 Space ownership
...
Tree root 本身就是一个普通 HSpace，不增加额外容器 HSpace。目录包含关系使从 root 工作的 Agent
...
Windows 保留名以及以 `.git`、`.harness-builder` 或 Agent managed root 开头的路径均拒绝。

所有 child path、realpath 和 logical name 在 portability comparison 下必须唯一，child name 不能
等于 root name。children 彼此不得包含；child 不得再声明 `harness-space-tree.json`。

任意普通 HSpace 均可在独立使用时成为 Tree root；但 v1 的同一棵 Tree 只支持一层 direct
children。child 继续拥有 children 属于递归 Tree，当前 schema/实现未支持。

## 3. CLI

```bash
python3 harnessbuilder.py check-tree [ROOT] [--offline] [--format text|json]
python3 harnessbuilder.py explain-tree [ROOT] [--offline] [--format text|json]
python3 harnessbuilder.py build-tree [ROOT] [--offline] [--dry-run] [--format text|json]
...
1. 获取 Tree root 的 `.harness-builder/tree-build.lock`；
...
较早成员的 post command 如果改变尚未构建的 child 输入，fresh plan 会以 `HB017` 拒绝。若多个
...
- root HSpace name、Tree manifest path 与 digest；
