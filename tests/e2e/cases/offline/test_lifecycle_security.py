...
class OwnershipLifecycleCases(PipeBuilderE2ECase):
    ...
    def test_deselecting_skill_removes_common_and_native_outputs(self):
        ...
        self.box.write_text("provider/temporary/.pipe-agents/cursor/.cursor/commands/temp.md", "TEMP\n")
        self.box.manifest(agents=["cursor"], skills=["temporary"], providers=[{"type": "folder", "path": "provider"}])
        self.expect_ok(self.box.builder("build"))
        manifest = json.loads((self.box.root / "pipespace.json").read_text())
        manifest["skills"] = []
    ...
    def test_deleted_managed_file_and_old_builder_version_converge_on_rebuild(self):
        ...
        self.box.write_json(".pipebuilder/lock.json", lock)
        self.expect_ok(self.box.builder("build"))
...
class LockAndInterruptionCases(PipeBuilderE2ECase):
    metadata = CaseMetadata(
        tier="offline",
        requirements=("PB013", "PB014", "INTERRUPTION", "CONCURRENCY"),
    ...
    def test_two_real_build_processes_fail_fast_for_the_loser(self):
        ...
        first = subprocess.Popen(
            argv,
            cwd=self.box.root,
            env=self.box.controlled_env({"PIPEBUILDER_TEST_HOLD_LOCK_MILLISECONDS": "1500"}),
            text=True,
            encoding="utf-8",
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
        lock = self.box.root / ".pipebuilder/build.lock"
        deadline = time.monotonic() + 5
        while not lock.exists() and time.monotonic() < deadline:
            time.sleep(0.01)
        self.assertTrue(lock.exists(), "first process did not acquire build.lock")
        second = self.box.builder("build")
        self.expect_code(second, "PB013")
    ...
    def test_injected_crash_leaves_stale_lock_and_manual_recovery_converges(self):
        crashed = self.box.builder("build", env={"PIPEBUILDER_TEST_CRASH_AFTER_WRITES": "1"})
        ...
        self.expect_code(self.box.builder("build"), "PB014")
        lock.unlink()
        self.expect_ok(self.box.builder("build"))
    ...
    def test_apply_failure_keeps_old_lock_and_next_build_converges(self):
        ...
        failed = self.box.builder("build", env={"PIPEBUILDER_TEST_FAIL_AFTER_WRITES": "1"})
        self.expect_code(failed, "PB011")
        self.assertEqual((self.box.root / ".pipebuilder/lock.json").read_bytes(), old_lock)
        self.assertFalse((self.box.root / ".pipebuilder/build.lock").exists())
        self.expect_ok(self.box.builder("build"))
        self.assertNotEqual((self.box.root / ".pipebuilder/lock.json").read_bytes(), old_lock)


class FilesystemBoundaryCases(PipeBuilderE2ECase):
    metadata = CaseMetadata(
        tier="offline",
        requirements=("PB010", "PB011", "PATH-SAFETY"),
    ...
    def test_owned_file_type_drift_is_preflighted_before_build_or_clean_changes_anything(self):
        ...
        before = snapshot_tree(self.box.root)
        self.expect_code(self.box.builder("build"), "PB010")
        self.assertEqual(snapshot_tree(self.box.root), before)
    ...
    def test_invalid_existing_lock_is_not_trusted_or_replaced(self):
        self.box.manifest(agents=["codex"])
        self.box.write_text(".pipebuilder/lock.json", "{}\n")
        before = snapshot_tree(self.box.root)
        self.expect_code(self.box.builder("build"), "PB001")
        self.assertEqual(snapshot_tree(self.box.root), before)
    ...
    def test_forged_lock_cannot_claim_or_clean_a_human_owned_file(self):
        ...
        before = snapshot_tree(self.box.root)
        self.expect_code(self.box.builder("clean"), "PB001")
        self.assertEqual(snapshot_tree(self.box.root), before)
    ...
    def test_builder_state_directory_symlink_is_rejected_before_lock_write(self):
        ...
        process = subprocess.Popen(
            argv,
            cwd=self.box.root,
            env=self.box.controlled_env({"PIPEBUILDER_TEST_HOLD_LOCK_MILLISECONDS": "750"}),
            text=True,
            encoding="utf-8",
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        ...
        self.assertIn("PB011", stdout)
        self.assertFalse(observed_outside_write, "build.lock was created outside the PipeSpace")
        self.assertEqual(list(outside.iterdir()), [])
