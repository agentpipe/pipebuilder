...
状态：implemented
基线日期：2026-07-13
最近验证：E0 57/57，Codex E1 5/5，Codex E2 1/1
...
| Tier | 当前覆盖 | 外部依赖 | 默认门禁 |
| --- | --- | --- | --- |
| E0 offline | 57 个 case，约 120 个正负场景 | Python 3.11+、真实文件系统 | PR 必跑 |
...
| provider/selection | `test_providers_skills.ProviderResolutionCases` | 零/缺失/multiple provider、local 优先、显式+tag+local 并集、shadow provenance、provider 顺序 |
| common Skill | `test_providers_skills.SkillPackageCases` | binary、hidden、executable、unknown frontmatter、`.DS_Store`、`.harness-agents` 排除、symlink、invalid Skill |
...
| adapter negatives | `test_adapters.AdapterRejectionCases` | gated/unknown surface、malformed JSON/TOML/frontmatter、semantic conflict、portable case collision、secret lint |
| ownership/clean | `test_lifecycle_security.OwnershipLifecycleCases` | clean 只删 owned、幂等、rebuild、反选 Skill、移除 Agent、source update、无 lock 不猜 ownership |
...
| filesystem security | `test_lifecycle_security.FilesystemBoundaryCases` | unowned target、type drift 全量 preflight、target symlink escape、case collision、invalid lock、recursive provider |
...
| HB005 | missing folder provider | covered |
...
| HB012 | `adapter-not-implemented` | reserved；四个 v1 agent 均已实现，当前没有可达输入，不伪造假 case |
...
2. Skill body 提供 Skill sentinel，生成的 `AGENTS.md` 提供另一个 sentinel；
